...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
         ...
         */
        List<String> itemListOnSale = shopStream.flatMap(a->a.getItemList().stream()).map(a->a.getName()).collect(Collectors.toList());
        Set<String> itemSetNotOnSale = customerStream.flatMap(a->a.getWantToBuy().stream()).map(a->a.getName()).filter(a->!itemListOnSale.contains(a)).collect(Collectors.toSet());
    ...
    public void havingEnoughMoney() {
        ...
        List<Item> onSale = shopStream.flatMap(a->a.getItemList().stream()).collect(Collectors.toList());
